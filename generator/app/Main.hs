...
parse::[Char] -> IO [Char]
parse xs = if xs == [] then generate else toIO xs --чтобы возвращал одно и то же (IO) делаем toIO логина
...
maslina:: Ref TextBuffer -> Ref Button -> IO()
maslina login _  = do
 res <- generate
 setText login (pack $ res)
 return()
...
main = do
  ...
  login <- textBufferNew Nothing Nothing
  edit <- textEditorNew (toRectangle (150,100,200,30)) Nothing
  setBuffer edit (Just login)
  
  butt <- buttonNew (toRectangle (360, 100, 80, 25))  (Just "GENERATE")
  setColor butt (Color 255)
  setCallback butt (maslina login)
